...
export function ExecutionPanel({ pair, onPlaceTrade }: ExecutionPanelProps) {
  ...
  const liqPrice = side === 'Long' 
    ...
    : orderPrice * (1 + 1/leverage);
  const executionFee = sizeUsd * 0.0012; // 0.12% fee
  ...
  return (
    <section className="bg-white p-3 md:p-4 rounded-[24px] dream-shadow shrink-0">
      ...
      <div ref={leverageRef}>
        <div className="flex gap-2 mb-4">
          <motion.button 
            ...
            className="flex-1 py-1.5 px-2 bg-white hover:bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-[#004282] transition-colors"
          >
            {marginMode}
          </motion.button>
          ...
          <motion.button 
            ...
            className={`flex-1 py-1.5 px-2 border rounded-lg text-xs font-bold transition-colors flex items-center justify-center gap-1 ${
              showLeverageDropdown 
                ? 'bg-white border-slate-300 text-[#004282]' 
                : 'bg-white hover:bg-slate-50 border-slate-200 text-[#004282]'
            }`}
          >
            {leverage}x
          </motion.button>
            ...
            >
              <div className="p-4 bg-white border border-slate-200 rounded-2xl shadow-sm">
                <div className="flex justify-between items-center mb-3">
                  <span className="text-sm font-bold text-slate-500">Leverage</span>
                ...
                <input 
                  ...
                  className="w-full accent-dream-blue h-2 bg-slate-100 rounded-lg appearance-none cursor-pointer mb-3" 
                />
      ...
      <div className="relative flex p-1 bg-white rounded-2xl mb-4 border border-slate-200">
        <motion.div 
        ...
        <motion.button 
          ...
          className={`relative z-10 flex-1 py-2.5 rounded-xl font-extrabold text-sm transition-colors ${
            side === 'Long' ? 'text-white' : 'text-slate-500 hover:text-slate-700'
          }`}
        >
          Long
        </motion.button>
        <motion.button 
          ...
          className={`relative z-10 flex-1 py-2.5 rounded-xl font-extrabold text-sm transition-colors ${
            side === 'Short' ? 'text-white' : 'text-slate-500 hover:text-slate-700'
          }`}
        >
          Short
        </motion.button>
      ...
      {/* Market / Limit Tabs */}
      <div className="flex gap-4 mb-4 border-b border-slate-100 px-1">
        <motion.button 
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          onClick={() => setOrderType('Market')}
          className={`pb-2 text-sm font-bold relative transition-colors ${
            orderType === 'Market' ? 'text-slate-900' : 'text-slate-400 hover:text-slate-600'
          }`}
        >
          Market
          {orderType === 'Market' && (
            <motion.div layoutId="activeOrderTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-slate-900 rounded-t-full" />
          )}
        </motion.button>
        <motion.button 
          whileHover={{ scale: 1.05 }}
          ...
          className={`pb-2 text-sm font-bold relative transition-colors ${
            orderType === 'Limit' ? 'text-slate-900' : 'text-slate-400 hover:text-slate-600'
          }`}
        >
          Limit
          {orderType === 'Limit' && (
      ...
      <div className="flex justify-between items-center text-xs mb-2 px-1">
        ...
        <span className="font-bold text-slate-700">10,000.00 USDC</span>
      </div>
      ...
      <div className="space-y-3 mb-4">
          ...
          {orderType === 'Limit' && (
            ...
            >
              <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
                <span className="text-slate-400 font-bold text-sm">Price</span>
              </div>
              <motion.input 
                ...
                className="w-full py-3 pl-16 pr-16 bg-white border border-slate-200 rounded-xl font-bold text-sm text-right text-slate-900 focus:border-dream-blue/30 focus:ring-2 focus:ring-dream-blue/20 outline-none transition-all" 
              />
              <div className="absolute inset-y-0 right-4 flex items-center pointer-events-none">
                <span className="text-slate-600 font-bold text-sm">USDC</span>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <motion.div className="relative" whileFocus={{ scale: 1.02 }}>
          ...
          <input 
            ...
            className="w-full py-3 pl-16 pr-20 bg-white border border-slate-200 rounded-xl font-bold text-sm text-right text-slate-900 focus:border-dream-blue/30 focus:ring-2 focus:ring-dream-blue/20 outline-none transition-all" 
          />
      ...
      <div className="flex items-center gap-3 mb-4 px-1">
        <input 
          ...
          className="flex-1 accent-dream-blue h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer" 
        />
        <motion.div 
          key={sizePercent}
          initial={{ scale: 0.8, opacity: 0.5 }}
          animate={{ scale: 1, opacity: 1 }}
          className="px-2 py-1 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-600 w-12 text-center"
        >
          {sizePercent}%
        </motion.div>
      </div>
      ...
      <div className="space-y-2.5 mb-5 px-1">
        <label className="flex items-center gap-2.5 cursor-pointer group">
          <motion.div 
            whileTap={{ scale: 0.8 }}
            className={`w-4 h-4 rounded flex items-center justify-center transition-colors ${reduceOnly ? 'bg-dream-blue' : 'bg-slate-200 group-hover:bg-slate-300'}`}
          >
          ...
          <span className="text-xs font-bold text-slate-500 group-hover:text-slate-700 transition-colors">Reduce-only</span>
          <input type="checkbox" className="hidden" checked={reduceOnly} onChange={(e) => setReduceOnly(e.target.checked)} />
        </label>
        <label className="flex items-center gap-2.5 cursor-pointer group">
          <motion.div 
            whileTap={{ scale: 0.8 }}
            className={`w-4 h-4 rounded flex items-center justify-center transition-colors ${tpsl ? 'bg-dream-blue' : 'bg-slate-200 group-hover:bg-slate-300'}`}
          >
          ...
          <span className="text-xs font-bold text-slate-500 group-hover:text-slate-700 transition-colors">TP/SL</span>
      ...
      <div className="space-y-1.5 py-2 px-1 mb-4">
        <div className="flex justify-between text-[11px] md:text-xs">
          <span className="text-slate-400 font-medium">Liquidation Price</span>
          <span className="font-bold text-slate-700">{formatCurrency(liqPrice)}</span>
        </div>
        <div className="flex justify-between text-[11px] md:text-xs">
          <span className="text-slate-400 font-medium">Execution Fee</span>
          <span className="font-bold text-slate-700">{formatCurrency(executionFee)}</span>
        </div>
      </div>
