...
const LiquidationRiskGauge = ({ risk, skipAnimation }: { risk: number; skipAnimation?: boolean }) => {
   const totalBars = 20;
   const [displayedRisk, setDisplayedRisk] = React.useState(skipAnimation ? risk : 100);

   React.useEffect(() => {
      if (skipAnimation) {
         setDisplayedRisk(risk);
         return;
      }
      ...
      let timeout = setTimeout(() => {
         ...
         const animate = (time: number) => {
            ...
            if (phase === 0) {
               ...
               setDisplayedRisk(100 - ease * 100);

               if (progress >= 1) {
                  phase = 1;
                  start = time; // Use exact time for smooth transition
               }
   ...
   const activeBars = Math.floor((displayedRisk / 100) * totalBars);
   const currentAngle = displayedRisk * 1.8 - 90;
      ...
      <div className="relative flex flex-col items-center justify-end w-full mt-6" style={{ height: '110px' }}>
         <svg viewBox="0 0 200 100" className="w-[85%] overflow-visible">
            {Array.from({ length: totalBars }).map((_, i) => {
               ...
               // Active: light blue -> solid blue. Inactive: weak gray -> slightly stronger gray
               const activeColor = `rgba(51, 102, 255, ${0.4 + 0.6 * ratio})`;
               const inactiveColor = `rgba(148, 163, 184, ${0.1 + 0.2 * ratio})`;
            ...
            {/* Needle */}
            <g style={{ transform: `rotate(${currentAngle}deg)`, transformOrigin: '100px 100px' }}>
...
export function Dashboard({ positions, onNavigate }: { positions: Position[], onNavigate: (tab: string) => void }) {
   ...
   const hasSeenAnim = useStore((state) => state.hasSeenDashboardAnimation);
   const setHasSeenAnim = useStore((state) => state.setHasSeenDashboardAnimation);
   const [skipAnimation] = React.useState(hasSeenAnim);

   React.useEffect(() => {
      if (!skipAnimation) {
         setHasSeenAnim(true);
      }
   }, [skipAnimation, setHasSeenAnim]);
   ...
   const containerVariants = {
      ...
      show: {
         opacity: 1,
         transition: { staggerChildren: skipAnimation ? 0 : 0.1 }
      }
   };
   ...
   return (
      <div className="flex flex-col h-full w-full">
         ...
         <div className="flex justify-between items-center mb-6 shrink-0 px-2 mt-4">
            <div>
               <h1 className="text-3xl font-bold text-dm-text">Hey trader, here are your stats </h1>
            </div>
         </div>
         ...
         >
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 pb-6">
               ...
               <motion.div variants={cardVariants} className="bg-dm-surface border border-dm-border rounded-[24px] p-6 flex flex-col relative overflow-hidden">
                  ...
                  <div className="flex items-center justify-center mt-6">
                     <motion.div
                        ...
                        transition={{ type: "spring", stiffness: 200, damping: 20, delay: skipAnimation ? 0 : 0.2 }}
                        className="w-20 h-20 rounded-full bg-[#3366FF] flex items-center justify-center text-white font-bold relative z-10 shadow-lg shadow-[#3366FF]/20"
                     >
                        55%
                     </motion.div>
                     <motion.div
                        initial={skipAnimation ? false : { scale: 0, opacity: 0, x: -20 }}
                        animate={{ scale: 1, opacity: 1, x: 0 }}
                        transition={{ type: "spring", stiffness: 200, damping: 20, delay: skipAnimation ? 0 : 0.3 }}
                        className="w-28 h-28 rounded-full bg-[#3366FF]/30 flex items-center justify-center text-white font-bold relative z-0 -ml-8 shadow-lg backdrop-blur-md border border-[#3366FF]/30"
                     >
                        35%
                     </motion.div>
                     <motion.div
                        initial={skipAnimation ? false : { scale: 0, opacity: 0, x: -10 }}
                        animate={{ scale: 1, opacity: 1, x: 0 }}
                        transition={{ type: "spring", stiffness: 200, damping: 20, delay: skipAnimation ? 0 : 0.4 }}
                        className="w-16 h-16 rounded-full bg-dm-surface-strong border border-dm-border flex items-center justify-center text-dm-text2 font-bold relative z-10 -ml-4 shadow-lg"
                     >
                        10%
                     </motion.div>
                  </div>
               ...
               <motion.div variants={cardVariants} className="bg-dm-surface border border-dm-border rounded-[24px] p-6 flex flex-col relative overflow-hidden">
                  ...
                  <LiquidationRiskGauge risk={liquidationRisk} skipAnimation={skipAnimation} />
               </motion.div>
               ...
               <motion.div variants={cardVariants} className="bg-dm-surface border border-dm-border rounded-[24px] p-6 flex flex-col relative overflow-hidden">
                  ...
                  <div className="flex w-full gap-2 mt-auto pb-4 overflow-hidden">
                     <motion.div
                        initial={skipAnimation ? false : { width: 0, opacity: 0 }}
                        animate={{ width: `${marginUsage}%`, opacity: 1 }}
                        transition={{ duration: skipAnimation ? 0 : 1, ease: "easeOut", delay: skipAnimation ? 0 : 0.1 }}
                        className="flex flex-col gap-2 min-w-[fit-content]"
                     >
                        <motion.span initial={skipAnimation ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: skipAnimation ? 0 : 0.9 }} className="text-lg font-bold text-dm-text">{marginUsage.toFixed(0)}%</motion.span>
                        <div className="h-8 w-full bg-[#3366FF] rounded-[8px]"></div>
                     </motion.div>
                     <motion.div
                        initial={skipAnimation ? false : { width: 0, opacity: 0 }}
                        animate={{ width: '25%', opacity: 1 }}
                        transition={{ duration: skipAnimation ? 0 : 1, ease: "easeOut", delay: skipAnimation ? 0 : 0.3 }}
                        className="flex flex-col gap-2 min-w-[fit-content]"
                     >
                        <motion.span initial={skipAnimation ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: skipAnimation ? 0 : 1.1 }} className="text-lg font-bold text-dm-text">2%</motion.span>
                        <div className="h-8 w-full bg-[#3366FF]/40 rounded-[8px]"></div>
                     </motion.div>
                     <motion.div
                        initial={skipAnimation ? false : { width: 0, opacity: 0 }}
                        animate={{ width: `${Math.max(0, 100 - marginUsage - 2)}%`, opacity: 1 }}
                        transition={{ duration: skipAnimation ? 0 : 1, ease: "easeOut", delay: skipAnimation ? 0 : 0.5 }}
                        className="flex flex-col gap-2 flex-1 min-w-[fit-content]"
                     >
                        <motion.span initial={skipAnimation ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: skipAnimation ? 0 : 1.3 }} className="text-lg font-bold text-dm-text">{Math.max(0, 100 - marginUsage - 2).toFixed(0)}%</motion.span>
               ...
               <motion.div variants={cardVariants} className="bg-dm-surface border border-dm-border rounded-[24px] p-6 flex flex-col relative overflow-hidden">
                  ...
                  <motion.div
                     ...
                     transition={{ duration: skipAnimation ? 0 : 0.8, ease: "easeOut", delay: skipAnimation ? 0 : 0.3 }}
                     className="w-full h-32 mt-auto relative"
                  >
